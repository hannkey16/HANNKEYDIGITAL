...
const highlights = [
  {
    ...
    caption: 'Langsung dikirim otomatis',
  },
  {
    icon: ShieldCheck,
    title: 'Aman & Terpercaya',
    caption: '100% garansi uang kembali',
  },
  {
    icon: Tag,
    title: 'Harga Terbaik',
    caption: 'Lebih murah setiap hari',
  },
]
      ...
      <div className="relative overflow-hidden rounded-[calc(var(--radius)*1.15)] border border-white/8 bg-[#080d1f]/90">
        ...
        <div className="flex items-center gap-3 border-b border-white/6 px-3 py-3 sm:px-4">
          <div className="hidden w-[188px] shrink-0 items-center gap-2 md:flex">
            <span className="grid size-7 place-items-center rounded-lg bg-brand-gradient text-[11px] font-bold text-white">
              H
            </span>
          ...
          <div className="relative shrink-0">
            ...
            <span className="absolute -top-1.5 -right-1.5 grid size-3.5 place-items-center rounded-full bg-brand text-[8px] font-bold text-white">
              3
            </span>
          </div>

          <div className="hidden items-center gap-2 sm:flex">
            <span className="grid size-7 place-items-center rounded-full bg-brand-gradient text-[10px] font-bold text-white">
              H
            </span>
            <span className="text-[12px] text-muted-foreground">Hi, Premium</span>
        ...
        <div className="flex">
          {/* Sidebar */}
          <aside className="hidden w-[188px] shrink-0 flex-col gap-0.5 border-r border-white/6 p-2.5 md:flex">
            {sidebarItems.map((item) => (
              <div
                key={item.label}
                className={cn(
                  'flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[12px] transition-colors',
                  item.active
                    ? 'border border-brand/25 bg-brand/12 font-medium text-foreground'
                    : 'text-muted-foreground hover:bg-white/4',
                )}
              >
                <item.icon
                  className={cn('size-3.5 shrink-0', item.active && 'text-brand')}
                />
          ...
          <div className="min-w-0 flex-1 p-3 sm:p-4">
            <div className="mb-3 flex items-end justify-between gap-3">
              <h3 className="text-[13px] font-semibold sm:text-sm">
                Kategori Populer
              </h3>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
              {products.map((product, index) => (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="gradient-border group rounded-2xl border border-white/7 bg-white/3 p-2.5 transition-colors hover:bg-white/6"
                >
                  <span
                    className="mb-2 grid aspect-[1.7/1] w-full place-items-center rounded-xl"
                  ...
                  <p className="text-[12px] font-semibold text-brand-2">
